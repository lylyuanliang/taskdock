...
- 使用 SQLite 在本地保存数据；已在当前开发分支实现 WebDAV 双向同步，支持默认端到端加密、冲突处理和暂停/恢复。
...
M4 WebDAV 同步尚未随已发布的 `v0.2.0` 安装包发布。当前开发分支只支持一个 WebDAV 配置和一个远端当前快照；正式发布前还需要使用真实 WebDAV 服务完成 Windows 安装版人工验收。
...
- [M4 WebDAV E2E 验证](tests/e2e/sync-flow.md)：同步设置、冲突和暂停/恢复的自动化验证边界。
...
- 当前开发分支的同步只使用 WebDAV。WebDAV 密码和加密口令由 Windows Credential Manager 保存，不写入源码、SQLite、远端快照或提交记录。
- AI 集成仍属于后续工作。不要将任何服务商凭据写入源码或提交记录。
